...
def parse_markdown(md):
    """Extract numbered references from a markdown '## References' section (lines 'N. ...')."""
    block = md.split("## References", 1)[1] if "## References" in md else md
    refs = re.findall(r"^\d+\.\s+(.*)$", block, re.M)
    return [(str(i + 1), r) for i, r in enumerate(refs)]
...
def dc_by_doi(doi, mailto):
    """DataCite fallback for DOIs CrossRef does not index (Zenodo, OSF, Dryad, figshare)."""
    body, status = _get(f"{DATACITE}/{urllib.parse.quote(doi)}", mailto)
    if not body or status == 404:
        return None
    try:
        a = json.loads(body)["data"]["attributes"]
        titles = a.get("titles") or []
        return (titles[0].get("title") if titles else ""), str(a.get("publicationYear") or "")
    except Exception:
        return None
...
def check_entry(key, raw, mailto):
    ...
    if e["doi"]:
        ...
        if msg is None:
            dc = dc_by_doi(e["doi"], mailto)
            if dc is not None:
                mt, my = dc
                sc = containment(mt, e["text"]) if mt else None
                r.update(verdict="OK", matched_title=mt, score=(round(sc, 2) if sc is not None else None),
                         detail=f"DataCite DOI resolves (title match={round(sc, 2) if sc is not None else 'n/a'}, year={my})")
                return r
            r.update(verdict="FLAG", detail=f"DOI does not resolve on CrossRef or DataCite: {e['doi']}")
            return r
...
def main():
    ...
    entries = (parse_markdown(tex) if a.path.endswith('.md')
               else parse_bibfile(tex) if a.path.endswith('.bib')
               else parse_thebibliography(tex))
    if not entries:
